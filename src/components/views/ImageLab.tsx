...
export function ImageLab({ scene, onClose }: ImageLabProps) {
  ...
  const handleAnalyze = () => {
    setIsAnalyzing(true);
    addLog('info', `Image Lab: Analyzing scene ${scene.sceneNumber} composition...`);

    setTimeout(() => {
      setIsAnalyzing(false);
      setAnalysis(
        `**Composition Analysis — Scene ${scene.sceneNumber}**\n\n` +
        `• **Framing:** Wide establishing shot with strong depth of field\n` +
        `• **Lighting:** High-contrast rim lighting with neon ambient fill\n` +
        `• **Color Palette:** Cool blues/indigos with warm accent highlights\n` +
        `• **Mood:** Cinematic, futuristic, atmospheric\n` +
        `• **Suggested Improvements:** Consider adding volumetric fog for depth, slightly warm the highlights for contrast`
      );
      addLog('success', `Image Lab: Analysis complete for scene ${scene.sceneNumber}`);
    }, 2000);
  };
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-foreground">Image Lab</h2>
                <p className="text-xs text-muted-foreground font-mono">Scene {scene.sceneNumber} • {scene.sceneType}</p>
              </div>
            </div>
          ...
          <div className="flex max-h-[calc(90vh-64px)]">
            ...
            <div className="flex-1 flex items-center justify-center p-6 border-r border-border">
              <div className="relative w-full aspect-video rounded-md bg-gradient-to-br from-indigo-900/30 to-purple-900/30 border border-border overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <Eye className="mx-auto h-12 w-12 text-primary/30 mb-2" />
                    <p className="text-sm text-muted-foreground">Scene {scene.sceneNumber} Preview</p>
                ...
                {isEditing && (
                  ...
                  >
                    <div className="text-center">
                      ...
                      <p className="text-sm text-primary font-medium">Applying edit...</p>
                      <p className="text-xs text-muted-foreground mt-1">Processing with gemini-2.5-flash-image</p>
                    </div>
                  </motion.div>
                )}
              </div>
            ...
            <div className="w-80 flex flex-col overflow-auto">
              ...
              <div className="border-b border-border p-4">
                <h3 className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  <Wand2 className="h-3 w-3" /> Edit Image
                </h3>
                <textarea
                  value={editPrompt}
                  onChange={(e) => setEditPrompt(e.target.value)}
                  placeholder="e.g. Make it night time, add rain..."
              ...
              <div className="border-b border-border p-4">
                <h3 className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  <ScanSearch className="h-3 w-3" /> Analyze
                </h3>
                <button
                ...
                {analysis && (
                  <motion.div
                    ...
                    className="mt-3 rounded-md border border-border bg-background p-3 text-xs text-foreground/80 whitespace-pre-line leading-relaxed"
                  >
                    {analysis}
                  </motion.div>
                )}
              ...
              <div className="flex-1 p-4">
                <h3 className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  <RotateCcw className="h-3 w-3" /> Edit History
                </h3>
                {editHistory.length === 0 ? (
                  <p className="text-xs text-muted-foreground/50">No edits yet</p>
                ) : (
